<?php
include	'global.php';
/**
 * Page model config
 */

return array(

	'title' => "Charity Fund >>> Charity Fund(ALL)",

	'single' => "charityfund",

	'model' => 'Charityfund',
	
	'form_width' => 1000,
	
	/** Disable the Create and Delete button	
	'action_permissions'=> array(
	'delete' => function($model)
	{
	return false;
	},
	
	'create' => function($model)
	{
	return false;
	},

	),
	**/
	/*Rules*/
	'rules' => array(
    'title_eng' => 'required',
        'title_cht' => 'required',

	    'title_chs' => 'required',
	'date' => 'required',
	

	),
	
	/*Messages*/
	'messages' => array(
    'title_eng.required' => 'The Title(ENG) is required',
   	'date.required' => 'The Date is required',
	),
	
	/**
	 * The display columns
	 *  	 	'image' => array(
    	'title' => "Image",
  		'type'=>'image',
     	'output' => function($value)
    	{
    	if($value){
    	list($width, $height)=getimagesize($GLOBALS['home_directory'].'app/storage/uploads/'.$value);
        return '<p>'.$value.'</p><img src="'.$GLOBALS['home_directory'].'app/storage/uploads/'.$value.'" height="100" /><p>Dimension: '.$width.'(w) x '.$height.'(h)</p>';
		}else{
		return '<p>No Image</p>';	
		}	
		
		},
		),	
	 */
	'columns' => array(
	
		'title_eng'=> array(
        'title' => 'Title(ENG)'        
    	),

    	'date'=> array(
        'title' => 'date'        
    	),

	),


/**
 * The sort options for a model
 *
 * @type array
 */
'sort' => array(
    'field' => 'id',
    'direction' => 'asc',
),


	/**
	 * The filter set
	 */
		'filters' => array(
		'title_eng'=> array(
        'title' => 'Title(ENG)'
        
    	),
       	'date'=> array(
        'title' => 'date'        
    	),
		
    
    	
	),

	/**
	 * The editable fields
	 */
	 
	 
		'edit_fields' => array(
		
			'date'=> array(
        	'type' => 'date',
   			'title' => 'Date',
    	
    		),
    
		'title_eng'=> array(
        'title' => 'Title(ENG)'
    	),
    	'title_cht'=> array(
        'title' => 'Title(CHT)'
    	),
    		'title_chs'=> array(
        'title' => 'Title(CHS)'
    	),

		'zoom_in_detail_eng'=> array(
        	'title' => 'Zoom In Detail(ENG)',
			'type' => 'wysiwyg'
    	),
    	'zoom_in_detail_cht'=> array(
        	'title' => 'Zoom In Detail(CHT)',
			'type' => 'wysiwyg'
    	),
		'zoom_in_detail_chs'=> array(
        	'title' => 'Zoom In Detail(CHS)',
			'type' => 'wysiwyg'
    	),

		'img1' => array(
  		'title' => 'Image 1',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
    	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
        array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
		'img2' => array(
  		'title' => 'Image 2',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
    	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
        array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
		'img3' => array(
  		'title' => 'Image 3',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
             array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
		'img4' => array(
  		'title' => 'Image 4',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
    		'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
              array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
		'img5' => array(
  		'title' => 'Image 5',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
             array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img6' => array(
  		'title' => 'Image 6',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
  	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
           array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img7' => array(
  		'title' => 'Image 7',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
    	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
             array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img8' => array(
  		'title' => 'Image 8',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
    		'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
        array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img9' => array(
  		'title' => 'Image 9',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
            array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img10' => array(
  		'title' => 'Image 10',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
   	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
               array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img11' => array(
  		'title' => 'Image 11',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
   	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
          array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img12' => array(
  		'title' => 'Image 12',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
  	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
              array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img13' => array(
  		'title' => 'Image 13',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
    	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
           array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img14' => array(
  		'title' => 'Image 14',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
        array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img15' => array(
  		'title' => 'Image 15',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
    		'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
            array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img16' => array(
  		'title' => 'Image 16',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
    		'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
        array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img17' => array(
  		'title' => 'Image 17',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
           array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img18' => array(
  		'title' => 'Image 18',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
                array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img19' => array(
  		'title' => 'Image 19',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
    	'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
            array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
			'img20' => array(
  		'title' => 'Image 20',
   		'type' => 'image',
   		'location' => storage_path() . '/uploads/thumbs/',
    		'description' => 'Suggested image size: 440(W) x 264(H)',
   		'naming' => 'random',
    	'mimes' => 'jpg,jpeg,png,bmp,gif',
		'sizes' => array(
            array(440, 264, 'fit', storage_path() . '/uploads/thumbs/full/',100)
    	)
		),
		
	

   
	),
	
);